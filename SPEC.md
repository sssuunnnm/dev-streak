...
GitHub token 생성 시 branch를 선택하는 것이 아니라, 앱이 GitHub API를 호출할 때 확인할 branch/ref를 정한다.

현재 MVP는 `main` branch와 open Pull Request commit을 확인한다. Working branch 직접 등록/검사는 현재 MVP에 포함하지 않는다. 후속 버전에서 추가할 수 있도록 GitHub API client와 verification service는 ref 기반 조회를 유지한다.
...
- GitHub token에서 검증 branch를 선택하지 않는다.
...
- 최초 GitHub 연결 후 history sync는 최근 3년의 commit history를 자동으로 한 번 확인한다.
